...
func (s *Service) Work(job int) {
	s.Lock.RLock() // Con RLock las lecturas no estaran bloqueadas.
	...
	if exists {
		...
		resp := <-response // Bloquea el programa
	...
	if exists {
		for _, pendingWorker := range pendingWorkers {
			pendingWorker <- result // Notifica a  todos los workers que estaban pendientes ya esta listo el resultado,
		} // y se les estan enviando.
